...
public class Pizarra extends JFrame implements Runnable {
	...
	private void initComponents() {
			...
			public void keyPressed(KeyEvent e) {
				...
				switch (e.getExtendedKeyCode()) {
					case VK_W -> {
						...
						logger.append("[W ↑ Arriba] " + selectedModel.getNombre() + "\n");
					}
					case VK_S -> {// Down
						selectedModel.getPuntos().down(5d);
						logger.append("[S ↓ Abajo] " + selectedModel.getNombre() + "\n");
					}
					case VK_A -> {// Left
						selectedModel.getPuntos().left(5d);
						logger.append("[A ←] Izquierda" + selectedModel.getNombre() + "\n");
					}
					case VK_D -> {// Right
						selectedModel.getPuntos().right(5d);
						logger.append("[D →] Derecha" + selectedModel.getNombre() + "\n");
					}
					case VK_Q -> {// Rotate left
						selectedModel.getPuntos().rotateLeft(90);
						logger.append("[Q ↶] Rotar Izquierda" + selectedModel.getNombre() + "\n");
					}
					case VK_E -> {// Rotate right
						selectedModel.getPuntos().rotateRight(90);
						logger.append("[E ↷] Rotar Derecha" + selectedModel.getNombre() + "\n");
					}
					case VK_SHIFT -> { // Zoom in
						selectedModel.getPuntos().zoomIn(2d);
						logger.append("[Shift ←█→] Agrandar" + selectedModel.getNombre() + "\n");
					}
					case VK_CONTROL -> {// Zoom out
						selectedModel.getPuntos().zoomOut(2d);
						logger.append("[Ctrl →|←] Encoger" + selectedModel.getNombre() + "\n");
					}
				}
		...
		canvasPanel.setBorder(new LineBorder(Color.ORANGE, 2));
